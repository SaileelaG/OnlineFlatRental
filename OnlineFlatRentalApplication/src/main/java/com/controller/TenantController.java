...
//		
//		ResponseEntity re=new ResponseEntity<Tenant>(t1,HttpStatus.OK);
//		return re;
//	}
	@PutMapping(path="/update")
	public ResponseEntity<String> updateTenant( @RequestBody Tenant tenant) throws Exception
	{
		String t1=tenantservice.updateTenant(tenant);
	...
	public ResponseEntity<Optional<Tenant>> viewTenant(@PathVariable int tid) throws Exception
	{
		Optional<Tenant> le=tenantservice.viewTenant(tid);
		
		ResponseEntity<Optional<Tenant>> re=new ResponseEntity<Optional<Tenant>>(le,HttpStatus.OK);
		return re;
	}
